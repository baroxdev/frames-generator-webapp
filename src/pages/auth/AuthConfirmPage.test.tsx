...
describe("AuthConfirmPage", () => {
  ...
  it("shows a failure message and a link back to login when no session was established", async () => {
    mockUseAuthSession.mockReturnValue({
      session: null,
      user: null,
      isLoading: false,
    ...
    expect(
      screen.getByRole("link", { name: "Đến trang đăng nhập" }),
    ).toBeTruthy();
  });

  it("shows a success message and a continue link once the session is established", async () => {
    mockUseAuthSession.mockReturnValue({
