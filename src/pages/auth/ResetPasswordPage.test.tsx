...
describe("ResetPasswordPage", () => {
  ...
  it("shows a loading state while the recovery session is being verified", async () => {
  ...
  it("shows an invalid-link message and no form when there is no recovery session", async () => {
    mockUseAuthSession.mockReturnValue({
      session: null,
      user: null,
      isLoading: false,
    ...
    expect(
      screen.getByText(/Liên kết đặt lại mật khẩu không hợp lệ/),
    ).toBeTruthy();
  ...
  it("submits the new password and navigates to /account on success", async () => {
    ...
    await waitFor(() =>
      expect(mockUpdatePassword).toHaveBeenCalledWith(
        { newPassword: "new-correct-horse-1" },
        expect.anything(),
      ),
    );
    await waitFor(() =>
  ...
  it("reports a friendly error when updating the password fails", async () => {
    ...
    await waitFor(() =>
      expect(mockReportAuthError).toHaveBeenCalledWith(
        failure,
